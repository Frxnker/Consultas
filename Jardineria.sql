...
-- Lunes 4 de Enero de 2024
SET GLOBAL lc_time_names = 'es_Es';
SET lc_time_names = 'es_Es';

SELECT
	fecha_pedido,
    date_format(fecha_pedido, '%W %e de %M de %Y'),
    date_format(fecha_pedido, '%d-%m-%Y')
FROM pedido;

-- Calcula aquellos pedidos cuya fecha de entrega 
-- sea al menos 2 diás mayor de la esperada
SELECT *, datediff(fecha_entrega, fecha_esperada)
FROM pedido
WHERE datediff(fecha_entrega, fecha_esperada) >= 2
ORDER BY datediff(fecha_entrega, fecha_esperada) DESC;

SELECT DISTINCT YEAR(fecha_pedido)
FROM pedido;
...
-- 4
SELECT e1.puesto, e2.nombre, e2.apellido1, e2.apellido2, e2.email
FROM empleado e1
JOIN empleado e2 ON e1.codigo_jefe = e2.codigo_empleado
WHERE e1.codigo_jefe IS NULL;

-- 5
SELECT nombre, apellido1, apellido2, puesto
FROM empleado
WHERE puesto IS NOT NULL AND puesto <> 'Representante de Ventas';
